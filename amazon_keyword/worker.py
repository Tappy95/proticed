...
def run():
    ...
    server.add_routine_worker(update_rank_db_by_effecttask, interval=30, immediately=True)
    server.add_routine_worker(new_task_db_by_invalidtask, interval=60,immediately=True)
    print("run server")
    server.run()
    print("end")
